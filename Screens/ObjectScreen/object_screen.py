...
class Plate(MDCard):
    ...
    menu=None
    _video_title=StringProperty("2")
    _channel_name=StringProperty("1" )
...
#            self.ids.main.current = 'disconnect_network_3'  
    ...
    def stream_to_flask(self,*args):
        ...
        if state_recognition =='Stream':
            ...
            _, imdata = cv2.imencode('.JPG', frame_rgb)
            ip_stream = local[:-4] + '8080'
            ip= ip_stream+ '/upload'
    ...
    def human(self,*args):
        global data_get ,state_video_to_sign
        state_video_to_sign='/uploadermp3'
        data_get = "/gifdata"
    ...
    def vivian(self,*args):
        global data_get , state_video_to_sign
        state_video_to_sign = "/uploadermp3new"
        data_get = "/vivian"
    ...
    global state_video_to_sign    
    state_video_to_sign ='/uploadermp3'    
    def mp3_upload(self,*args):
        global local,filename,img, state_video_to_sign
        try:
            urlopen(local, timeout=1)
            print("Internet is active")
            myurl = local + state_video_to_sign
            files = {'file': open(self.pathname, 'rb')}
            getdata = requests.post(myurl, files=files)
